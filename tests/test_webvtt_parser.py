...
class WebVTTParserTestCase(GenericParserTestCase):
    def test_webvtt_parse_invalid_file(self):
        self.assertRaises(MalformedFileError, webvtt.read,
                          self._get_file("invalid.vtt"))

    def test_webvtt_captions_not_found(self):
        self.assertRaises(FileNotFoundError, webvtt.read, "some_file")

    def test_webvtt_total_length(self):
        self.assertEqual(
            webvtt.read(self._get_file("sample.vtt")).total_length, 64)
    ...
    def test_webvtt_parse_get_captions(self):
        self.assertEqual(
            len(webvtt.read(self._get_file("sample.vtt")).captions), 16)

    def test_webvtt_parse_invalid_timeframe_line(self):
        self.assertRaises(MalformedCaptionError, webvtt.read,
    ...
    def test_webvtt_timestamp_format(self):
        self.assertTrue(WebVTTParser()._validate_timeframe_line(
            "00:00:00.000 --> 00:00:00.000"))
        self.assertTrue(
            WebVTTParser()._validate_timeframe_line("00:00.000 --> 00:00.000"))
    ...
    def test_clean_cue_tags(self):
        ...
        self.assertEqual(vtt.captions[1].text, "Like a big-a pizza pie")
        self.assertEqual(vtt.captions[2].text, "That's amore")
    ...
    def test_can_parse_youtube_dl_files(self):
        ...
        self.assertEqual("this will happen is I'm telling",
                         vtt.captions[2].text)
