...
from webvtt import Caption, WebVTT, WebVTTSegmenter
...
class WebVTTSegmenterTestCase(unittest.TestCase):
    ...
    def test_invalid_captions(self):
        self.assertRaises(FileNotFoundError, self.segmenter.segment, "text")

        self.assertRaises(InvalidCaptionsError, self.segmenter.segment, 10)
    ...
    def test_segmentation_files_exist(self):
        ...
        self.assertTrue(
            os.path.exists(os.path.join(OUTPUT_DIR, "prog_index.m3u8")))
    ...
    def test_segment_content(self):
        ...
        with open(os.path.join(OUTPUT_DIR, "fileSequence0.webvtt"),
                  "r",
                  encoding="utf-8") as f:
    ...
    def test_manifest_content(self):
        ...
        with open(os.path.join(OUTPUT_DIR, "prog_index.m3u8"),
                  "r",
                  encoding="utf-8") as f:
            lines = [line.rstrip() for line in f.readlines()]
            ...
            for i in range(7):
                expected_lines.extend(
                    ["#EXTINF:30.00000", "fileSequence{}.webvtt".format(i)])
    ...
    def test_segment_from_file(self):
        self.segmenter.segment(os.path.join(SUBTITLES_DIR, "sample.vtt"),
                               OUTPUT_DIR),
        self.assertEqual(self.segmenter.total_segments, 7)

    def test_segment_with_no_captions(self):
        self.segmenter.segment(os.path.join(SUBTITLES_DIR, "no_captions.vtt"),
                               OUTPUT_DIR),
        self.assertEqual(self.segmenter.total_segments, 0)

    def test_total_segments_readonly(self):
        self.assertRaises(AttributeError, setattr, WebVTTSegmenter(),
                          "total_segments", 5)
